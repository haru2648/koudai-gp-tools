...
const functions = require("firebase-functions"); // configを読み込むために必要

// Firebaseアプリを初期化
admin.initializeApp();

// ★★★ 全ての関数のリージョンを東京に設定 (新しい書き方) ★★★
setGlobalOptions({ region: "asia-northeast1" });

// === ここからが「入国審査官」の本体 ===
// ★★★ onRequestの書き方が新しくなり、CORSの指定が簡単になった ★★★
exports.lineLoginCallback = onRequest({ cors: true }, async (req, res) => {
  try {
    // 1. ウェбサイトから送られてきた「通行証(code)」を取り出す
    ...
    const tokenResponse = await axios.post("https://api.line.me/oauth2/v2.1/token", new URLSearchParams({
      ...
      code: code,
      redirect_uri: functions.config().line.callback_url,
